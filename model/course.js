...
const addCourse = (ccode, cname, semester, year, status, numberofsection) => {
  return new Promise((resolve, reject) => {
    // const dataGrader = knex.pgGrader;
    knex.pgGrader("courses")
        .insert({
          ccode: ccode,
          cname: cname,
          semester: semester,
...
const updateCourseData = (cid, ccode, cname, semester, year, cstatus) => {
  return new Promise((resolve, reject) => {
    knex.pgGrader("courses")
        ...
        .then(data => { resolve({msg: "update Course Successful"}); })
        .catch(error => { reject({msg: "update Course Failed"}); });
  });
};

const getCoursesByStudentId = studentid => {
  return new Promise((resolve, reject) => {
    knex.pgGrader("students_in_courses")
...
const getCourseByAssignmentId = aid => {
  return new Promise((resolve, reject) => {
    knex.pgGrader("assignment_header")
        .where("aid", aid)
        .select("cid")
        .then(data => { resolve(data[0]); })
        .catch(error => { reject(error); });
...
const getStudentInCourseBySection = (cid, section) => {
  return new Promise((resolve, reject) => {
    knex.pgGrader("students_in_courses")
        .where({cid: cid, section: section})
        .join(
            "students", "students.studentid", "=",
            "students_in_courses.studentid")
        .select("students.studentid", "students.fullname")
...
const getCourseExist = (ccode, semester, year) => {
  return new Promise((resolve, reject) => {
    knex.pgGrader("courses")
        .where({ccode: ccode, semester: semester, year: year})
        .select()
        .then(data => {
          console.log(data);
          if (data.length <= 0) {
...
const getCourseSection = cid => {
  return new Promise((resolve, reject) => {
    knex.pgGrader("courses")
        .where("cid", cid)
        .select("numberofsection")
        .then(data => { resolve(data); })
        .catch(error => { reject(error); });
  });
};
...
module.exports = {
  ...
  getStudentInCourseBySection: getStudentInCourseBySection,
